...
func handleAdminUserActions(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    data := callback.Data
    if strings.HasPrefix(data, "adjust_") {
        targetUserIDStr := strings.TrimPrefix(data, "adjust_")
        targetUserID, _ := strconv.ParseInt(targetUserIDStr, 10, 64)
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("💰 *Enter amount to adjust \\(+ for add, - for deduct\\):*"))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        updateUser(db, User{UserID: userID, State: "adjusting_balance_" + targetUserIDStr})
    } else if strings.HasPrefix(data, "ban_") {
        targetUserIDStr := strings.TrimPrefix(data, "ban_")
        targetUserID, _ := strconv.ParseInt(targetUserIDStr, 10, 64)
    ...
    } else if strings.HasPrefix(data, "unban_") {
        targetUserIDStr := strings.TrimPrefix(data, "unban_")
        targetUserID, _ := strconv.ParseInt(targetUserIDStr, 10, 64)
        targetUser, err := getUser(db, targetUserID)
        if err == nil && targetUser.UserID != 0 {
            targetUser.Banned = 0
    ...
    } else if strings.HasPrefix(data, "viewrefs_") {
        targetUserIDStr := strings.TrimPrefix(data, "viewrefs_")
        targetUserID, _ := strconv.ParseInt(targetUserIDStr, 10, 64)
        targetUser, err := getUser(db, targetUserID)
        if err == nil && targetUser.UserID != 0 {
    ...
    } else if strings.HasPrefix(data, "contact_") {
        targetUserIDStr := strings.TrimPrefix(data, "contact_")
        targetUserID, _ := strconv.ParseInt(targetUserIDStr, 10, 64)
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("📩 *Enter message to send to user* %d:", targetUserID))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        updateUser(db, User{UserID: userID, State: "contacting_" + targetUserIDStr})
    }
    bot.Request(tgbotapi.NewCallback(callback.ID, ""))
}
    ...
    case "withdraw_amount":
        ...
        if qrEnabled == "1" {
            ...
            if err != nil {
                ...
                msg := tgbotapi.NewMessage(0, notification+"\n⚠️ *QR code generation failed\\!*")
                msg.ChatID = paymentChannel // Assign string directly
                msg.ParseMode = "MarkdownV2"
                msg.ReplyMarkup = markup
                bot.Send(msg)
                bot.Send(tgbotapi.NewMessage(ADMIN_ID, formatMarkdownV2("⚠️ *QR code generation failed for withdrawal from user %d\\!*", userID)))
            } else {
                photo := tgbotapi.NewPhoto(0, tgbotapi.FileBytes{Name: "qr_withdrawal.png", Bytes: qrBytes})
                photo.ChatID = paymentChannel // Assign string directly
