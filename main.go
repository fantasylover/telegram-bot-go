...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "broadcast_message":
        ...
        statusMsg, _ := bot.Send(initialMsg)

        var broadcastMsg tgbotapi.Chattable
        if update.Message.Photo != nil && len(update.Message.Photo) > 0 {
            photo := update.Message.Photo[len(update.Message.Photo)-1]
            broadcastMsg = tgbotapi.NewPhoto(0, tgbotapi.FileID(photo.FileID))
            ...
            broadcastMsg = tgbotapi.NewVideo(0, tgbotapi.FileID(update.Message.Video.FileID))
            if update.Message.Caption != "" {
        ...
        for _, uid := range users {
            if photoMsg, ok := broadcastMsg.(*tgbotapi.PhotoConfig); ok {
                photoMsg.ChatID = uid
            } else if videoMsg, ok := broadcastMsg.(*tgbotapi.VideoConfig); ok {
                videoMsg.ChatID = uid
            } else if textMsg, ok := broadcastMsg.(*tgbotapi.MessageConfig); ok {
                textMsg.ChatID = uid
            }
            bot.Send(broadcastMsg)
            sentCount++
    ...
    case "remove_channel":
        ...
        updateUser(db, user)
    default:
        // Handle dynamic states with if-else
