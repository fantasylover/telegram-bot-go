...
func handleStart(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update) {
    ...
    // Check required channels and show main menu
    ...
    // Show main menu for returning users or after channel check
...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "support_message":
        ...
        // Clear state first to avoid getting stuck if sending fails
        user.State = ""
        if err := updateUser(db, user); err != nil {
        ...
        if _, err := bot.Send(adminMsg); err != nil {
            ...
            errorMsg := tgbotapi.NewMessage(userID, formatMarkdownV2("⚠️ *Support message submitted, but admin notification failed. We’ll address it soon!*"))
