...
func checkUserJoinedChannels(bot *tgbotapi.BotAPI, userID int64, db *sql.DB) (bool, error) {
    ...
    for _, channel := range channels {
        // Fetch numeric ChatID from channel username
        chat, err := bot.GetChat(tgbotapi.ChatInfoConfig{ChatConfig: tgbotapi.ChatConfig{ChatID: channel}})
        if err != nil {
            log.Printf("Error fetching chat ID for %s: %v", channel, err)
            return false, err
        }
        chatConfig := tgbotapi.GetChatMemberConfig{
            ChatConfigWithUser: tgbotapi.ChatConfigWithUser{
                ChatID: chat.ID, // Use numeric ID from GetChat
                UserID: userID,
            },
        }
...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "withdraw_amount":
        ...
        if paymentChannel != "" {
            ...
            chat, err := bot.GetChat(tgbotapi.ChatInfoConfig{ChatConfig: tgbotapi.ChatConfig{ChatID: paymentChannel}})
            if err != nil {
                log.Printf("Error fetching payment channel ID for %s: %v", paymentChannel, err)
                return
            }
            ...
            msgText := fmt.Sprintf(
                "🔥 *NEW WITHDRAWAL SENT* 🔥\n\n👤 *USER:* [%s](tg://user?id=%d)\n💎 *USER ID:* `%d`\n💰 *AMOUNT:* %s FREE COIN\n📞 *REFERRER:* %d\n🔗 *ADDRESS:* `%s`\n⏰ *TRANSACTION ID:* `%s`",
                escaped późFirstName, userID, userID, escapeMarkdownV2(amountStr), user.Referrals, escapedWallet, txID,
            )
    ...
    case "broadcast_message":
        ...
        if err != nil {
            log.Printf("Error sending broadcast status: %v", err)
            return
        }
    ...
    case "add_channel":
        ...
        // Validate channel by fetching Chat info, but don't store numeric ID
        if _, err := bot.GetChat(tgbotapi.ChatInfoConfig{ChatConfig: tgbotapi.ChatConfig{ChatID: channel}}); err != nil {
