...
func handleStart(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update) {
    ...
    if user.UserID == 0 {
        ...
        if referredBy != 0 {
            ...
            if err == nil && referrer.UserID != 0 {
                referralReward, _ := strconv.ParseFloat(getSetting(db, "referral_reward"), 64)
    ...
    startMessage := getSetting(db, "start_message")
    if startMessage == "" {
        startMessage = "🎉 Welcome to the Referral & Earning Bot! Join channels to start."
    }
    escapedStartMessage := escapeMarkdownV2(startMessage) // Escape the message
