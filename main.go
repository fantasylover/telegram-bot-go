...
func escapeMarkdownV2(text string) string {
    // List of MarkdownV2 special characters that need escaping unless part of formatting
    specialChars := []string{"[", "]", "(", ")", "~", ">", "#", "+", "-", "=", "|", "{", "}", ".", "!"}
...
func formatMarkdownV2(template string, args ...interface{}) string {
    // Format the string with arguments first
    formatted := template
    if len(args) > 0 {
        formatted = fmt.Sprintf(template, args...)
    }
    // Escape only non-formatting special characters, preserving MarkdownV2 syntax
