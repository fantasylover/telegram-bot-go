...
func handleAdminUserActions(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    if userID != ADMIN_ID {
        ...
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "🚫 Unauthorized"))
        return
    }
    ...
    if len(parts) != 2 {
        ...
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Invalid callback data!*"))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "❌ Invalid"))
        return
    }
    action := parts[0]
    ...
    if err != nil {
        log.Printf("Error parsing target user ID from callback %s: %v", data, err)
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Invalid user ID in callback!*"))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "❌ Invalid ID"))
        return
    }
    ...
    if err != nil || targetUser.UserID == 0 {
        ...
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "❌ Not Found"))
        return
    }
    ...
    case "adjust":
        ...
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        if err := updateUser(db, User{UserID: userID, State: "adjusting_balance_" + targetUserIDStr}); err != nil {
            log.Printf("Error setting state for user %d: %v", userID, err)
            bot.Send(tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Error setting adjustment state!*")))
        }
    case "ban":
        targetUser.Banned = 1
        if err := updateUser(db, targetUser); err != nil {
            log.Printf("Error banning user %d: %v", targetUserID, err)
            bot.Send(tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Error banning user!*")))
        } else {
    ...
    case "unban":
        ...
        if err := updateUser(db, targetUser); err != nil {
            ...
            bot.Send(tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Error unbanning user!*")))
        } else {
    ...
    case "viewrefs":
        ...
        if err != nil {
            ...
            bot.Send(tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Error fetching referrals!*")))
            bot.Request(tgbotapi.NewCallback(callback.ID, "❌ Error"))
            return
        }
        defer rows.Close()
    ...
    case "contact":
        ...
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        if err := updateUser(db, User{UserID: userID, State: "contacting_" + targetUserIDStr}); err != nil {
            log.Printf("Error setting state for user %d: %v", userID, err)
            bot.Send(tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Error setting contact state!*")))
        }
    }
    bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Processed"))
}
...
func handleAdminStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "adjusting_balance_":
        ...
        if err != nil || amount < 0 {
            log.Printf("Invalid amount %s for user %d: %v", adjustment, targetUserID, err)
            msg := tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Invalid amount!*"))
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            return
        ...
        if err := updateUser(db, targetUser); err != nil {
            ...
            log.Printf("Balance adjusted for user %d: New balance %.2f", targetUserID, targetUser.Balance)
    ...
    case "add_channel":
        channel := strings.TrimSpace(update.Message.Text)
        if !strings.HasPrefix(channel, "@") {
            msg := tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Channel must start with '@' (e.g., @ChannelName)!*"))
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            return
        }
        ...
        if err != nil {
            ...
            msg := tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Error checking channels!*"))
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            return
        }
        for _, ch := range channels {
            if ch == channel {
                msg := tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Channel* %s *already added!*", channel))
                msg.ParseMode = "MarkdownV2"
                bot.Send(msg)
                user.State = ""
                updateUser(db, user)
    ...
    case "remove_channel":
        ...
        if !found {
            msg := tgbotapi.NewMessage(userID, formatMarkdownV2("❌ *Channel* %s *not found!*", channel))
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            user.State = ""
            updateUser(db, user)
...
// --- End of Part 10B: Handle State Messages (Admin States) and Admin Actions ---
