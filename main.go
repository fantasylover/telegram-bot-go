...
func handleCallbackQuery(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    if callback.Data == "set_inline" || callback.Data == "set_normal" {
        ...
        // Show confirmation with option to change style again
        markup := tgbotapi.NewInlineKeyboardMarkup(
            tgbotapi.NewInlineKeyboardRow(
                tgbotapi.NewInlineKeyboardButtonData("🔄 Change Style", "change_style"),
                tgbotapi.NewInlineKeyboardButtonData("➡️ Continue", "back_to_menu"),
            ),
        )
