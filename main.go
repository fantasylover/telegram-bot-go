...
func handleCallbackQuery(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    case "view_referrals":
        ...
        if len(referrals) == 0 {
            msgText = formatMarkdownV2("📄 *Your referrals:* %d\n*Balance:* %.2f 💰\n*No referrals yet!*", user.Referrals, user.Balance)
        }
...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "broadcast_message":
        ...
        if update.Message.Photo != nil && len(update.Message.Photo) > 0 {
            ...
            fileID = photo.FileID
            photoMsg := tgbotapi.NewPhoto(0, tgbotapi.FileID(fileID))
            ...
            for _, uid := range users {
                ...
                if _, err := bot.Send(photoMsg); err != nil {
                    log.Printf("Error sending photo broadcast to user %d: %v", uid, err)
                    continue
                }
                sentCount++
                updateBroadcastProgress(bot, userID, statusMsg.MessageID, sentCount, totalUsers)
                time.Sleep(100 * time.Millisecond)
            }
        } else if update.Message.Video != nil {
            fileID = update.Message.Video.FileID
            videoMsg := tgbotapi.NewVideo(0, tgbotapi.FileID(fileID))
            if update.Message.Caption != "" {
                videoMsg.Caption = update.Message.Caption
                videoMsg.ParseMode = "MarkdownV2"
            }
            for _, uid := range users {
                videoMsg.ChatID = uid
                if _, err := bot.Send(videoMsg); err != nil {
                    log.Printf("Error sending video broadcast to user %d: %v", uid, err)
                    continue
                }
                sentCount++
                updateBroadcastProgress(bot, userID, statusMsg.MessageID, sentCount, totalUsers)
            ...
            textMsg := tgbotapi.NewMessage(0, update.Message.Text)
            textMsg.ParseMode = "MarkdownV2"
            for _, uid := range users {
                textMsg.ChatID = uid
                if _, err := bot.Send(textMsg); err != nil {
                    log.Printf("Error sending text broadcast to user %d: %v", uid, err)
                    continue
                }
                sentCount++
                updateBroadcastProgress(bot, userID, statusMsg.MessageID, sentCount, totalUsers)
...
// Part 9B Starting
    ...
    case "setting_min_withdrawal":
        ...
        if _, err := strconv.ParseFloat(value, 64); err != nil {
            msg := tgbotapi.NewMessage(userID, "❌ *Enter a valid number!*")
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            return
        }
        updateSetting(db, "min_withdrawal", value)
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("✅ *Minimum Withdrawal set to:* %s", value))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        user.State = ""
        updateUser(db, user)
    case "setting_referral_reward":
        ...
        updateSetting(db, "referral_reward", value)
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("✅ *Referral Reward set to:* %s", value))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        user.State = ""
        updateUser(db, user)
    case "setting_start_message":
        ...
        value := update.Message.Text
        updateSetting(db, "start_message", value)
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("✅ *Start Message set to:* %s", value))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        user.State = ""
        updateUser(db, user)
    case "setting_payment_channel":
        ...
        if err := updateSetting(db, "payment_channel", channel); err != nil {
            ...
            msg := tgbotapi.NewMessage(userID, "❌ *Failed to set payment channel!*")
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            return
        }
    ...
    case "add_channel":
        ...
        if err := addRequiredChannel(db, channel); err != nil {
            ...
            msg := tgbotapi.NewMessage(userID, "❌ *Failed to add channel!*")
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            return
        }
    ...
    case "remove_channel":
        ...
        if err := removeRequiredChannel(db, channel); err != nil {
            ...
            msg := tgbotapi.NewMessage(userID, "❌ *Failed to remove channel!*")
            msg.ParseMode = "MarkdownV2"
            bot.Send(msg)
            return
        }
    ...
    default:
        if strings.HasPrefix(state, "adjusting_balance_") {
            ...
            if err != nil {
                log.Printf("Error parsing target user ID %s: %v", targetUserIDStr, err)
            ...
            if err != nil || targetUser.UserID == 0 {
                log.Printf("Error getting target user %d: %v", targetUserID, err)
                msg := tgbotapi.NewMessage(userID, "❌ *User not found!*")
                msg.ParseMode = "MarkdownV2"
                bot.Send(msg)
                user.State = ""
            ...
            if err != nil || amount < 0 {
                log.Printf("Error parsing adjustment amount %s: %v", adjustment, err)
            ...
            if err := updateUser(db, targetUser); err != nil {
                log.Printf("Error updating target user %d balance: %v", targetUserID, err)
                msg := tgbotapi.NewMessage(userID, "❌ *Failed to adjust balance!*")
                msg.ParseMode = "MarkdownV2"
                bot.Send(msg)
                return
            }
...
func updateBroadcastProgress(bot *tgbotapi.BotAPI, userID int64, messageID int, sentCount, totalUsers int) {
    ...
    editMsg := tgbotapi.NewEditMessageText(userID, messageID, formatMarkdownV2("📢 *Broadcasting:* [%s] %d%% (%d/%d)", bar, percentage, sentCount, totalUsers))
