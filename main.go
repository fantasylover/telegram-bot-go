...
func formatMarkdownV2(template string, args ...interface{}) string {
    // First, format the string with the arguments without escaping
    formatted := fmt.Sprintf(template, args...)
    // Then, escape the entire result
...
func showMainMenu(bot *tgbotapi.BotAPI, userID int64, buttonStyle string) {
    ...
    if buttonStyle == "inline" {
        ...
        markup = inlineMarkup
        // Remove keyboard only if switching to inline
        user, err := getUser(db, userID)
        if err == nil && user.ButtonStyle == "normal" {
            clearMsg := tgbotapi.NewMessage(userID, "")
        ...
        replyMarkup.OneTimeKeyboard = false // Persist the keyboard
        markup = replyMarkup
    }
...
func handleStart(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update) {
    ...
    if user.UserID == 0 { // New user
        ...
        if referredBy != 0 {
            ...
            if err != nil {
                log.Printf("Error fetching referrer %d for user %d: %v", referredBy, userID, err)
            } else if referrer.UserID == 0 {
                log.Printf("Referrer %d not found for user %d", referredBy, userID)
            } else {
                rewardStr, _ := getSetting(db, "referral_reward")
                reward, err := strconv.ParseFloat(rewardStr, 64)
                if err != nil {
                    reward = 5.0
                    log.Printf("Invalid referral reward setting '%s', defaulting to 5.0", rewardStr)
                }
                oldReferrals := referrer.Referrals
                oldBalance := referrer.Balance
                referrer.Balance += reward
                referrer.Referrals++
                if err := updateUser(db, referrer); err != nil {
                    log.Printf("Error updating referrer %d balance and referrals: %v", referredBy, err)
                } else {
                    updatedReferrer, err := getUser(db, referredBy)
                    if err != nil || updatedReferrer.Referrals != referrer.Referrals || updatedReferrer.Balance != referrer.Balance {
                        log.Printf("Referrer %d update verification failed: Expected Referrals %d, Balance %.2f; Got Referrals %d, Balance %.2f, Error: %v",
                            referredBy, referrer.Referrals, referrer.Balance, updatedReferrer.Referrals, updatedReferrer.Balance, err)
                    } else {
                        log.Printf("Referrer %d updated: Referrals %d -> %d, Balance %.2f -> %.2f", referredBy, oldReferrals, referrer.Referrals, oldBalance, referrer.Balance)
                    }
                    ...
                    if _, err := bot.Send(msg); err != nil {
                        log.Printf("Error notifying referrer %d: %v", referredBy, err)
                    }
                }
            }
            referredUser, err := getUser(db, userID)
            if err != nil || !referredUser.ReferredBy.Valid || referredUser.ReferredBy.Int64 != referredBy {
                log.Printf("Referral link not set correctly for user %d: Expected ReferredBy %d, Got %v, Error: %v", userID, referredBy, referredUser.ReferredBy, err)
            } else {
                log.Printf("User %d successfully linked to referrer %d", userID, referredBy)
            }
        }
    }

    if user.UserID != 0 && user.ButtonStyle == "" { // Only prompt if existing user has no style set
        ...
        if user.ButtonStyle == "" { // Fallback for new users who skip selection
            user.ButtonStyle = "inline" // Default to inline
            if err := updateUser(db, user); err != nil {
                log.Printf("Error setting default button style for user %d: %v", userID, err)
            }
        }
...
func handleCallbackQuery(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    if callback.Data == "set_inline" || callback.Data == "set_normal" {
        ...
        if !joined {
            ...
            showMainMenu(bot, userID, buttonStyle) // Show menu after style setup
        }
...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "support_message":
        ...
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        // Use corrected formatMarkdownV2 to handle escaping
        adminMsgText := formatMarkdownV2("📩 *Support message from* @%s (ID: %d):\n%s", user.Username, userID, supportMsg)
        adminMsg := tgbotapi.NewMessage(ADMIN_ID, adminMsgText)
