...
func escapeMarkdownV2(text string) string {
    // Updated to include all Telegram MarkdownV2 reserved characters
...
func formatMarkdownV2(template string, args ...interface{}) string {
    // Escape arguments individually before formatting
    if len(args) > 0 {
        escapedArgs := make([]interface{}, len(args))
        for i, arg := range args {
            switch v := arg.(type) {
            case string:
                escapedArgs[i] = escapeMarkdownV2(v)
            default:
                escapedArgs[i] = v
            }
        }
        return fmt.Sprintf(template, escapedArgs...)
    }
    return template
}
...
func showMainMenu(bot *tgbotapi.BotAPI, userID int64, buttonStyle string) {
    ...
    if buttonStyle == "inline" {
        ...
        replyMarkup.OneTimeKeyboard = false // Persistent keyboard like Python
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("✨ *Main Menu* ✨"))
        msg.ParseMode = "MarkdownV2"
...
func handleMenuOptions(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    log.Printf("Handling menu option '%s' for user %d", text, userID)

    switch text {
    case "💰 Balance":
        user, err := getUser(db, userID)
        if err != nil {
            log.Printf("Error fetching user %d for balance: %v", userID, err)
            sendError(bot, userID, ErrUserNotFound)
            return
        }
    ...
    case "💳 Set Wallet":
        if user.Wallet != "" {
            ...
            if err := updateUser(db, user); err != nil {
                ...
                sendError(bot, userID, ErrStateSetFailed)
            }
        }
    case "📞 Support":
    ...
    case "📈 Stats":
        ...
        if err != nil {
            log.Printf("Error fetching total users: %v", err)
            sendError(bot, userID, "E999")
            return
        }
        totalWithdrawals, err := getTotalWithdrawals(db)
        if err != nil {
            log.Printf("Error fetching total withdrawals: %v", err)
            sendError(bot, userID, "E999")
            return
        }
    ...
    default:
        log.Printf("Unhandled menu option '%s' for user %d", text, userID)
    }
}
...
func handleCallbackQuery(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    case "balance":
        ...
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Balance"))
    case "set_wallet":
        if user.Wallet != "" {
            msg := tgbotapi.NewMessage(userID, formatMarkdownV2("💳 *Your wallet:* `%s`", user.Wallet))
            msg.ParseMode = "MarkdownV2"
        ...
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Wallet"))
    case "change_wallet":
        ...
        user.State = "setting_wallet"
        updateUser(db, user)
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Change Wallet"))
    case "support":
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("📞 *Send your message for support:*"))
        msg.ParseMode = "MarkdownV2"
        bot.Send(msg)
        user.State = "support_message"
        updateUser(db, user)
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Support"))
    case "referral":
        referralLink := generateReferralLink(userID)
        msg := tgbotapi.NewMessage(userID, formatMarkdownV2("🔗 *Referral Link:* `%s`\n🤝 *Referrals:* %d", referralLink, user.Referrals))
        msg.ParseMode = "MarkdownV2"
        ...
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Referral"))
    case "view_referrals":
        ...
        msg.ReplyMarkup = backButton
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Referrals"))
    case "stats":
        totalUsers, err := getTotalUsers(db)
        if err != nil {
            log.Printf("Error fetching total users: %v", err)
        ...
        msg.ReplyMarkup = backButton
        bot.Send(msg)
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Stats"))
    case "withdraw":
        if user.Wallet == "" {
            msg := tgbotapi.NewMessage(userID, formatMarkdownV2("💳 *Set your wallet first!*"))
            msg.ParseMode = "MarkdownV2"
        ...
        bot.Request(tgbotapi.NewCallback(callback.ID, "✅ Withdraw"))
    }
