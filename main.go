...
func checkUserJoinedChannels(bot *tgbotapi.BotAPI, userID int64, db *sql.DB) (bool, error) {
    ...
    for _, channel := range channels {
        ...
        v := url.Values{}
        v.Add("chat_id", channel) // Pass the string username directly
...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "withdraw_amount":
        ...
        if paymentChannel != "" {
            ...
            v := url.Values{}
            v.Add("chat_id", paymentChannel) // Pass the string username
            resp, err := bot.Request("getChat", v)
            if err != nil {
    ...
    case "add_channel":
        ...
        v := url.Values{}
        v.Add("chat_id", channel) // Pass the string username
        resp, err := bot.Request("getChat", v)
        if err != nil {
