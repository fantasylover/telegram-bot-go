...
func checkUserJoinedChannels(bot *tgbotapi.BotAPI, userID int64, db *sql.DB) (bool, error) {
    ...
    for _, channel := range channels {
        ...
        params := url.Values{}
        params.Add("chat_id", channel) // Pass the string username directly
...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "withdraw_amount":
        ...
        if paymentChannel != "" {
            ...
            params := url.Values{}
            params.Add("chat_id", paymentChannel) // Pass the string username
            resp, err := bot.MakeRequest("getChat", params)
            if err != nil {
    ...
    case "add_channel":
        ...
        params := url.Values{}
        params.Add("chat_id", channel) // Pass the string username
        resp, err := bot.MakeRequest("getChat", params)
        if err != nil {
