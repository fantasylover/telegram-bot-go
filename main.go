...
func handleMenuOptions(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    userID := update.Message.From.ID
    log.Printf("Handling menu option for user %d, received text: %s", userID, update.Message.Text) // Log exact input

    // Check if user has joined required channels
    joined, err := checkUserJoinedChannels(bot, userID, db)
    if err != nil {
        log.Printf("Error checking channels for user %d: %v", userID, err)
        bot.Send(tgbotapi.NewMessage(userID, "❌ *Error checking channel status. Try again later.*"))
        return
    }
    ...
    if !joined {
        ...
        if err != nil {
            ...
            bot.Send(tgbotapi.NewMessage(userID, "❌ *Error fetching required channels.*"))
            return
        }
    ...
    // Handle menu options
    switch strings.TrimSpace(update.Message.Text) { // Trim spaces to handle variations
    case "💰 Balance":
        ...
        if _, err := bot.Send(msg); err != nil {
            ...
            bot.Send(tgbotapi.NewMessage(userID, "❌ *Error displaying balance. Try again.*"))
        }
    case "💳 Set Wallet":
        ...
        if user.Wallet != "" {
            ...
            if _, err := bot.Send(tgbotapi.NewMessage(userID, "💳 *Enter your wallet address:*")); err != nil {
    ...
    case "📞 Support":
        ...
        if _, err := bot.Send(tgbotapi.NewMessage(userID, "📞 *Send your message for support:*")); err != nil {
    ...
    case "💸 Withdraw":
        ...
        if user.Wallet == "" {
            if _, err := bot.Send(tgbotapi.NewMessage(userID, "💳 *Set your wallet first\\.*")); err != nil {
                log.Printf("Error sending wallet prompt to user %d: %v", userID, err)
            }
        } else {
            ...
            if user.Balance < minWithdrawal {
                ...
                if _, err := bot.Send(tgbotapi.NewMessage(userID, "💸 *Enter amount to withdraw:*")); err != nil {
...
func handleStateMessages(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user User) {
    ...
    case "support_message":
        ...
        bot.Send(tgbotapi.NewMessage(ADMIN_ID, fmt.Sprintf("📞 *Support from* @%s\n%s", escapeMarkdownV2(user.Username), escapeMarkdownV2(update.Message.Text))))
...
func handleAdminActions(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    case "admin_qr_settings":
        ...
        msg := tgbotapi.NewMessage(userID, fmt.Sprintf("🔳 *QR Status:* %s", qrStatus))
        msg.ParseMode = "MarkdownV2"
        msg.ReplyMarkup = markup
        bot.Send(msg)
    }
...
func handleQRSettings(bot *tgbotapi.BotAPI, db *sql.DB, callback *tgbotapi.CallbackQuery) {
    ...
    msg := tgbotapi.NewMessage(userID, fmt.Sprintf("🔳 *QR* %s", qrAction))
    msg.ParseMode = "MarkdownV2"
    bot.Send(msg)
    bot.Request(tgbotapi.NewCallback(callback.ID, ""))
}
